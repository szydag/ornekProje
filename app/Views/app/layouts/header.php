...
// User-detail sayfası için özel kontrol
if (preg_match('/^admin\/app\/user-detail\/[A-Za-z0-9+\/=]+$/', $currentUri)) {
    $pageTitle = 'Kullanıcı Detayı';
    $breadcrumbs = [
        ['title' => 'Ana Sayfa', 'url' => base_url('admin')],
        ['title' => 'Kullanıcılar', 'url' => base_url('admin/app/users')],
        ['title' => 'Kullanıcı Detayı', 'url' => null]
    ];
} elseif (preg_match('/^app\/user-detail\/\d+$/', $currentUri)) {
    $pageTitle = 'Kullanıcı Detayı';
    $breadcrumbs = [
        ['title' => 'Ana Sayfa', 'url' => base_url('app/home')],
        ['title' => 'Kullanıcılar', 'url' => base_url('app/users')],
        ['title' => 'Kullanıcı Detayı', 'url' => null]
    ];
} elseif (preg_match('/^apps\/materials\/[A-Za-z0-9+\/=]+$/', $currentUri)) {
    $pageTitle = 'İçerik Detayı';
    $breadcrumbs = [
        ['title' => 'Ana Sayfa', 'url' => base_url('/')],
        ['title' => 'İçeriklerim', 'url' => base_url('apps/my-materials')],
        ['title' => 'İçerik Detayı', 'url' => null]
    ];
} elseif (preg_match('/^admin\/apps\/materials\/[A-Za-z0-9+\/=]+$/', $currentUri)) {
    $pageTitle = 'İçerik Detayı';
    $breadcrumbs = [
        ['title' => 'Ana Sayfa', 'url' => base_url('admin')],
        ['title' => 'Eğitim İçerikleri', 'url' => base_url('admin/apps/materials')],
        ['title' => 'İçerik Detayı', 'url' => null]
    ];
} elseif (preg_match('/^app\/course-detail\/\d+$/', $currentUri)) {
    $pageTitle = 'Kurs Detayı';
    $breadcrumbs = [
        ['title' => 'Ana Sayfa', 'url' => base_url('app/home')],
        ['title' => 'Kurslar', 'url' => base_url('app/courses')],
        ['title' => 'Kurs Detayı', 'url' => null]
    ];
} elseif (preg_match('/^admin\/apps\/courses\/[A-Za-z0-9+\/=]+$/', $currentUri)) {
    $pageTitle = 'Kurs Detayı';
    $breadcrumbs = [
        ['title' => 'Ana Sayfa', 'url' => base_url('admin')],
        ['title' => 'Kurslar', 'url' => base_url('admin/apps/courses')],
        ['title' => 'Kurs Detayı', 'url' => null]
    ];
} else {
    switch ($currentUri) {
        case 'app/home':
            $pageTitle = 'Ana Sayfa';
            $breadcrumbs = [['title' => 'Ana Sayfa', 'url' => null]];
            break;
        case 'apps/my-materials':
            $pageTitle = 'İçeriklerim';
            $breadcrumbs = [
                ['title' => 'Ana Sayfa', 'url' => base_url('app/home')],
                ['title' => 'İçeriklerim', 'url' => null]
            ];
            break;
        case 'admin/apps/materials':
            $pageTitle = 'Eğitim İçerikleri';
            $breadcrumbs = [
                ['title' => 'Ana Sayfa', 'url' => base_url('app/home')],
                ['title' => 'Eğitim İçerikleri', 'url' => null],
            ];
            break;
        case 'admin/apps/materials/(:num)':
            $pageTitle = 'İçerik Detayı';
            $breadcrumbs = [
                ['title' => 'Ana Sayfa', 'url' => base_url('admin')],
                ['title' => 'Eğitim İçerikleri', 'url' => base_url('admin/apps/materials')],
        ...
        case 'admin/apps/courses':
            $pageTitle = 'Kursler';
            $breadcrumbs = [
                ['title' => 'Ana Sayfa', 'url' => base_url('app/home')],
                ['title' => 'Kursler', 'url' => null]
            ];
            break;
        case 'admin/apps/courses/(:num)':
            $pageTitle = 'Kurs Detayı';
            $breadcrumbs = [
                ['title' => 'Ana Sayfa', 'url' => base_url('admin')],
                ['title' => 'Kursler', 'url' => base_url('admin/apps/courses')],
                ['title' => 'Kurs Detayı', 'url' => null]
            ];
            break;
        ...
        case 'app/users':
            ...
            break;
        case 'app/add-material':
            $pageTitle = 'İçerik Ekle';
            $breadcrumbs = [
                ['title' => 'Ana Sayfa', 'url' => base_url('app/home')],
                ['title' => 'İçerik Ekle', 'url' => null]
            ];
            break;
        case 'app/my-profile':
            $pageTitle = 'Profilim';
            $breadcrumbs = [
                ['title' => 'Ana Sayfa', 'url' => base_url('app/home')],
                ['title' => 'Profilim', 'url' => null]
            ];
            break;
        case 'apps/reviewer-materials':
            $pageTitle = 'Değerlendirme';
            $breadcrumbs = [
                ['title' => 'Ana Sayfa', 'url' => base_url('app/home')],
                ['title' => 'Değerlendirme', 'url' => null]
            ];
            break;
        default:
            $pageTitle = 'Kurs';
...
<header class="kt-header fixed top-0 z-10 start-0 end-0 flex items-stretch shrink-0 bg-background" data-kt-sticky="true"
    ...
    <div class="kt-container-fixed flex justify-between items-stretch lg:gap-4" id="headerContainer">
        ...
        <div class="flex gap-2.5 lg:hidden items-center -ms-1">
            <a class="shrink-0" href="<?= base_url('app/home') ?>">
                <img class="max-h-[25px] w-full" src="assets/media/app/educontent-emblem.svg" />
            </a>
        ...
        <!-- End of Mobile Logo -->
        <!--Megamenu Contaoner-->
        <div class="hidden lg:flex items-center" id="breadcrumbsContainer">
            <!--Breadcrumbs-->
            <div class="flex items-center gap-1.25 text-sm font-medium">
                <?php foreach ($breadcrumbs as $index => $breadcrumb): ?>
                    <?php if ($index === count($breadcrumbs) - 1): ?>
                        <!-- Son eleman - aktif sayfa -->
                        <span class="text-mono font-medium text-primary">
                            <?= esc($breadcrumb['title']) ?>
                        </span>
                    <?php else: ?>
                        <!-- Önceki sayfalar -->
                        <?php if ($breadcrumb['url']): ?>
                            <a href="<?= $breadcrumb['url'] ?>" class="text-secondary-foreground hover:text-primary">
                                <?= esc($breadcrumb['title']) ?>
                            </a>
                        <?php else: ?>
        ...
        <div class="flex items-center gap-2.5">
            ...
            <div class="shrink-0" data-kt-dropdown="true" data-kt-dropdown-offset="10px, 10px"
                ...
                <div class="cursor-pointer shrink-0 flex items-center gap-2" data-kt-dropdown-toggle="true">
                    ...
                    $managementLabel = $isAdminSection ? 'Kullanıcı Paneli' : 'Yönetim';
                    $managementUrl = $isAdminSection ? base_url('/') : base_url('admin');
                    $managementIcon = $isAdminSection ? 'ki-home' : 'ki-setting-2';
                ...
                <div class="kt-dropdown-menu w-[250px]" data-kt-dropdown-menu="true">
                    <div class="flex items-center justify-between px-2.5 py-1.5 gap-1.5">
                        <div class="flex items-center gap-2">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="text-xs text-secondary-foreground font-medium leading-none">
                                    <?= session('user_email') ?: 'email@example.com' ?>
                                </div>
                            </div>
                        </div>
                    </div>
                    <ul class="kt-dropdown-menu-sub">
                        ...
                        <li>
                            <a class="kt-dropdown-menu-link" href="<?= base_url('app/my-profile') ?>">
                        ...
                        <?php if ($hasManagementAccess): ?>
                            <li>
                                <a class="kt-dropdown-menu-link" href="<?= $managementUrl ?>">
    <i class="ki-filled <?= $managementIcon ?>"></i>
    <?= $managementLabel ?>
</a>
                    ...
                    <div class="px-2.5 pt-1.5 mb-2.5 flex flex-col gap-3.5">
                        ...
                        <button class="kt-btn kt-btn-outline justify-center w-full"
                            data-kt-modal-toggle="#logout_confirmation_modal">
                            Çıkış Yap
                        </button>
                    </div>
                </div>
